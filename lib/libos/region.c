...
bool regionCanJoin(const struct MemRegion *region1,
                   const struct MemRegion *region2)
{
  return regionDoesOverlap(region1, region2)
      || regionIsAdjacent(region1, region2);
}
...
bool regionDoesOverlap(const struct MemRegion *region1,
                       const struct MemRegion *region2)
{
  return regionDoesContain(region2->start, region1)
      || regionDoesContain(region1->start, region2);
}

/* Indicates whether one region ends exactly where the other region begins. */

bool regionIsAdjacent(const struct MemRegion *region1,
                      const struct MemRegion *region2)
{
