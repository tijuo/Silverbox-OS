...
struct tm* gmtime(const time_t *timer) {
  ...
  while(t >= (_isleap(1900 + __timestruct.tm_year) ? 366 : 365)) {
    ...
    __timestruct.tm_wday = (__timestruct.tm_wday
        + (_isleap(1900 + __timestruct.tm_year) ? 366 : 365))
                           % 7;
  ...
  for(int i = 0; i < 12; i++, __timestruct.tm_mon++) {
    if(t < (unsigned)(month_days[i]
        + (i == 1 && _isleap(1900 + __timestruct.tm_year) ? 1 : 0))) {
