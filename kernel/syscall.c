...
static long int handle_sys_receive(tid_t sender, unsigned long int flags);
static long int handle_sys_send(tid_t recipient, long int subject, unsigned long int flags);
...
static long int handle_sys_create_thread(void *entry, paddr_t addr_space,
        void *stack_top, cap_token_t token);
static long int handle_sys_destroy_thread(tid_t tid, cap_token_t token);
static long int handle_sys_read_thread(tid_t tid, thread_info_t *info, cap_token_t token);
static long int handle_sys_update_thread(tid_t tid, unsigned long int, thread_info_t *info, cap_token_t token);

static long int handle_sys_grant(tid_t grantee_tid, cap_token_t grantor_token, cap_rights_t flag_mask, cap_token_t *grantee_token);
static long int handle_sys_revoke(tid_t grantee_tid, cap_token_t grantor_token);
static long int handle_sys_poll(unsigned long int mask);
static long int handle_sys_wait(unsigned long int mask);
...
long int handle_sys_grant(tid_t grantee_tid, cap_token_t grantor_token, cap_rights_t flag_mask, cap_token_t *grantee_token) {
    tcb_t *current_thread = get_current_thread();
    tcb_t *grantee = get_tcb(grantee_tid);

    if(grantee_token == NULL || current_thread == grantee || grantee == NULL)
        RET_MSG(ESYS_ARG, "Invalid argument.");

    switch(cap_grant(current_thread, grantee, grantor_token, flag_mask, grantee_token)) {
        case E_OK:
            return ESYS_OK;
        case E_PERM:
            return ESYS_PERM;
...
long int handle_sys_revoke(tid_t grantee_tid, cap_token_t grantor_token) {
    tcb_t *current_thread = get_current_thread();
    tcb_t *grantee = get_tcb(grantee_tid);

    if(current_thread == grantee || grantee == NULL)
        RET_MSG(ESYS_ARG, "Invalid argument.");

    switch(cap_revoke(current_thread, grantor_token)) {
        case E_OK:
            return ESYS_OK;
        case E_PERM:
            return ESYS_PERM;
...
long int handle_sys_poll(unsigned long int mask) {
    tcb_t *current_thread = get_current_thread();
...
long int handle_sys_wait(unsigned long int mask) {
    tcb_t *current_thread = get_current_thread();
    long int ret_val = current_thread->pending_events & mask;
...
int handle_sys_get_page_mappings(addr_t virt, size_t count, paddr_t addr_space, struct PageMapping *mappings, unsigned int level) {
...
#define LEVEL       (unsigned int)args.sub_arg.word
    ...
    if(addr_space == CURRENT_ROOT_PMAP) {
        if(level == 2) {
            ...
            addr_space = (paddr_t)get_root_page_map();
    }

    switch(level) {
        case 0:
            for(i = 0; i < count && virt < KERNEL_VSTART;
                    i++, virt += PAGE_SIZE, mappings++) {
...
long int handle_sys_create_thread(void *entry, paddr_t addr_space, void *stack_top, cap_token_t token) {
    tcb_t *current_thread = get_current_thread();
    paddr_t root_pmap =
    ...
    .tid = NULL_TID
        ...
        RET_MSG(
            ESYS_PERM,
            "Calling thread doesn't have permission to create a new thread.");
}
...
long int handle_sys_read_thread(tid_t tid, thread_info_t *info, cap_token_t token) {
    tcb_t *current_thread = get_current_thread();
...
long int handle_sys_update_thread(tid_t tid, unsigned long int flags, thread_info_t *info, cap_token_t token) {
    tcb_t *current_thread = get_current_thread();
...
long int handle_sys_send(tid_t recipient, long int subject, unsigned long int flags) {
    tcb_t *current_thread = get_current_thread();
...
long int handle_sys_receive(tid_t sender, unsigned long int flags) {
    tcb_t *current_thread = get_current_thread();
