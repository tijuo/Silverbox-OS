...
int paging_map_page(addr_t virt, paddr_t phys, long int flags, paddr_t root_pmap) {
  ...
  for(unsigned int paging_level = num_paging_levels; paging_level;
      paging_level--) {
    entry = &table[(virt >> (PAGE_BITS + PAE_ENTRY_BITS * (paging_level - 1)))
                    & (PAE_PMAP_ENTRIES-1)];

    if(!IS_FLAG_SET(*entry, PAE_PRESENT)) {
      volatile pmap_entry_t *new_table;

      if(paging_level == 1)
        new_table = (volatile pmap_entry_t *)phys;
      else {
        new_table = (volatile pmap_entry_t *)kalloc_page();

        if(new_table == NULL)
          RET_MSG(E_FAIL, "Unable to allocate page map table.");

        clear_phys_page((paddr_t)new_table);
      }

      *entry = (pmap_entry_t)new_table;
      SET_FLAG(*entry, flags | PAE_PRESENT);
      table = new_table;
      continue;
    }
    else if(IS_FLAG_SET(*entry, PAE_PS))
      RET_MSG(E_OVERWRITE, "Address is already mapped to a large page");
    else if(paging_level == 1)
