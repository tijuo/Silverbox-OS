...
C_STUB="crt0"
CFLAGS="-O0 -Wall -m64 -std=gnu11 -fno-builtin -nostdlib -ffreestanding -nostartfiles -nodefaultlibs"
ASFLAGS="-O0 -f elf64"
...
LDFLAGS="-melf_x86_64 --exclude-libs ALL -T $SB_PREFIX/cLink.ld --static -L$GCCLIBS -( -lc -los -lgcc -)"
...
if [ $# -lt 2 ]; then
  ...
  x86_64-elf-gcc -c `for i in $INCLUDES; do echo -n "-I $i "; done` $CFLAGS $TOOLS/$C_STUB.c -o $TOOLS/$C_STUB.o
#fi
...
x86_64-elf-gcc -c `for i in $INCLUDES; do echo -n "-I $i "; done` $CFLAGS $files
x86_64-elf-ld $obj $TOOLS/$C_STUB.o `for l in $LIBS; do echo -n "-L $l "; done` $LDFLAGS -o $output
