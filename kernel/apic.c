...
void send_apic_eoi(void) {
  volatile unsigned int *eoi_reg = (volatile unsigned int*)(LAPIC_EOI
      + LAPIC_VADDR);
...
void init_apic_timer(void) {
  volatile unsigned int *timer_reg = (volatile unsigned int*)(LAPIC_TIMER
      + LAPIC_VADDR);
  volatile unsigned int *div_conf_reg = (volatile unsigned int*)(LAPIC_TIMER_DCR
      + LAPIC_VADDR);
...
#define IA32_APIC_BASE MSR = 0x1Bu;
  ...
  kMapPage((addr_t)LAPIC_VADDR, (addr_t)LAPIC_BASE,
