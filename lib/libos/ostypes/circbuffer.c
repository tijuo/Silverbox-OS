...
size_t readCircBuffer(struct CircularBuffer *buffer, size_t bytes, void *outData)
{
  ...
  size_t upperSize = (size_t)(((uintptr_t)buffer->data + buffer->bufLen) - (uintptr_t)buffer->ptr);

  if(bytesToRead > upperSize)
  {
    memcpy(outData, buffer->ptr, upperSize);
    memcpy((void *)((uintptr_t)outData + upperSize), buffer->data, bytesToRead-upperSize);
...
{
  size_t bytesToWrite = bytes > buffer->bufLen - buffer->unreadLen
  ...
  if(bytesToWrite > upperSize)
  {
    ...
    memcpy(buffer->data, (void *)((uintptr_t)data + upperSize), bytesToWrite-upperSize);
  }
  else
